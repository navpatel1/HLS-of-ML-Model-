...
for (qi = 0; qi < output->numel; qi++) {
    ...
    size_t qi;
for (qi = 0; qi < input->numel; qi++) {
    output->array[offset + qi] = input->array[qi];
}
...
for (qi = 0; qi < output->numel; qi++) {
    ...
    for (size_t i=0; i<in_height; ++i) {
        ...
        size_t qi;
for (qi = 0; qi < num; qi++) {
    output->array[offset + qi] = input->array[i * num + qi];
}
...
for (qi = 0; qi < output->numel; qi++) {
    ...
    for (size_t i=0; i<dim1; ++i) {
        for (size_t j=0; j<dim2; ++j) {
            ...
            size_t qi, qj;
for (qi = 0; qi < num; qi++) {
    for (qj = 0; qj < sizeof(input->array[0]); qj++) {
        output->array[offset1 + i * outstep1 + j * outstep2 + qi * sizeof(input->array[0]) + qj] = input->array[i * instep1 + j * instep2 + qi * sizeof(input->array[0]) + qj];
    }
}
...
void k2c_crop1d(k2c_tensor* output, const k2c_tensor* input, const size_t * crop) {
    ...
    size_t qi;
for (qi = 0; qi < output->numel; qi++) {
    output->array[qi] = input->array[offset + qi];
}
...
void k2c_crop2d(k2c_tensor* output, const k2c_tensor* input, const size_t * crop) {
    ...
    for (size_t i=0; i<out_height; ++i) {
        ...
        size_t qi;
for (qi = 0; qi < num; qi++) {
    output->array[i * num + qi] = input->array[offset + qi];
}
...
void k2c_crop3d(k2c_tensor* output, const k2c_tensor* input, const size_t * crop) {
    ...
    for (size_t i=0; i<outdim1; ++i) {
        for (size_t j=0; j<outdim2; ++j) {
            ...
            size_t qi;
for (qi = 0; qi < num; qi++) {
    output->array[i * outstep1 + j * outstep2 + qi] = input->array[offset1 + i * instep1 + j * instep2 + qi];
}
...
void k2c_upsampling2d(k2c_tensor* output, const k2c_tensor* input, const size_t * size) {
    ...
    for (size_t i=0; i<out_height; ++i) {
        for (size_t j=0; j<out_width; ++j) {
            ...
            size_t qi;
size_t output_idx = k2c_sub2idx(outsub, output->shape, output->ndim);
size_t input_idx = k2c_sub2idx(insub, input->shape, input->ndim);

for (qi = 0; qi < channels; qi++) {
    output->array[output_idx + qi] = input->array[input_idx + qi];
}
